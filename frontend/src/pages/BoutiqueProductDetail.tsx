...
import { ArrowLeft, ShoppingCart, Plus, Minus, Star, Share2, Check, ShoppingBag, X, ChevronLeft, ChevronRight, Store } from 'lucide-react'
...
const BoutiqueProductDetail = () => {
  const { slugOrId, productId } = useParams<{ slugOrId: string; productId: string }>()
  const navigate = useNavigate()
  const { isDark } = useTheme()
  const [boutique, setBoutique] = useState<Boutique | null>(null)
  const [product, setProduct] = useState<BoutiqueProduct | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [quantity, setQuantity] = useState(1)
  const [relatedProducts, setRelatedProducts] = useState<BoutiqueProduct[]>([])
  ...
  const loadProduct = async () => {
    if (!slugOrId || !productId) return
    try {
      setLoading(true)
      setError(null)

      const isNumeric = /^\d+$/.test(slugOrId)
      const boutiqueId = isNumeric ? Number(slugOrId) : null
      
      if (!boutiqueId && boutique?.id) {
        const response = await boutiquesApi.getProduct(boutique.id, Number(productId))
        ...
        const allProductsResponse = await boutiquesApi.getProducts(boutique.id)
        const allProducts = allProductsResponse.data.data || []
        const related = allProducts
          .filter((item: BoutiqueProduct) => 
        ...
        const response = await boutiquesApi.getProduct(boutiqueId, Number(productId))
        const foundProduct = response.data.data
        ...
        const allProductsResponse = await boutiquesApi.getProducts(boutiqueId)
        const allProducts = allProductsResponse.data.data || []
        const related = allProducts
          .filter((item: BoutiqueProduct) => 
            item.category === foundProduct.category && 
  ...
  // Recharger le produit quand la boutique est chargée
  useEffect(() => {
    if (boutique?.id && !product) {
      loadProduct()
    }
  }, [boutique?.id]) // eslint-disable-line react-hooks/exhaustive-deps

  // Réinitialiser l'index de l'image quand le produit change
  useEffect(() => {
  ...
  const addToCart = () => {
    if (!product || product.stock === 0 || !boutique) return
  ...
  return (
    <>
      <SEO
        title={`${product.name} - ${boutique.name}`}
        description={product.description || `Découvrez ${product.name} sur ${boutique.name}`}
        image={product.image || undefined}
        url={`/boutiques/${boutique.slug || boutique.id}/products/${product.id}`}
      />
        ...
        <div className="container-custom max-w-7xl">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <motion.button
              ...
              className="relative px-6 py-3 rounded-lg bg-gradient-primary text-white font-semibold flex items-center space-x-2 hover:shadow-glow transition-all transform hover:scale-105"
            >
              ...
              <div className="space-y-4 pt-4">
                ...
                <div className="flex gap-4">
                  ...
                  <button
                    ...
                    className="px-6 py-3 rounded-lg glass-effect text-white [data-theme='light']:text-dark-500 hover:bg-white/10 transition-colors"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {relatedProducts.map((relatedProduct) => (
                  <motion.div
                    ...
                    whileHover={{ y: -5 }}
                    onClick={() => navigate(`/boutiques/${boutique.slug || boutique.id}/products/${relatedProduct.id}`)}
...
export default BoutiqueProductDetail
